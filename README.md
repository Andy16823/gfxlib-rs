...
fn main() {
    ...
    //Create an texture2d_shader for the texture rendering
    ...
    let mut texture_batch_shader = Texture2DBatchShader::build_shader_program();
    window.render_device.create_shader_program(&mut texture_batch_shader);

    let mut solid_rect_shader = SolidRectShader::build_shader_program();
    window.render_device.create_shader_program(&mut solid_rect_shader);

    let mut rect_shader = RectShader::build_shader_program();
    window.render_device.create_shader_program(&mut rect_shader);
    ...
    let mut smiley = ImageTexture::load_from_file("C:/Users/andy1/Downloads/smiley.png");
    window.render_device.load_texture(&mut smiley);
    ...
    let mut static_transform = Transform3D::new(Vector3::new(0.0, 0.0, 0.0), Vector3::new(0.0, 0.0, 0.0), Vector3::new(64.0, 64.0, 0.0));

    
    let mut ellapsed_time = Instant::now();
    ...
    while !window.should_close() {
        ...
        window.render_device.set_viewport(viewport);
        window.render_device.set_camera(&mut camera);

        // Render the animated sprite to its render target.
        window.render_device.resize_render_target(&mut animated_sprite_rt, viewport.size.x, viewport.size.y);
        ...
        window.render_device.draw_sub_texture2d(player_transform.clone(), Vector2::new(current_frame.x, current_frame.y), Vector2::new(current_frame.widht, current_frame.height), &mut image, Vector4::new(1.0, 1.0, 1.0, 1.0)); //draw the texture as an subtexture
        window.render_device.unbind_shader_program();
        window.render_device.unbind_render_target();

        // Render static sprites to the sprite render target.
        window.render_device.resize_render_target(&mut sprite_rt, viewport.size.x, viewport.size.y);
        window.render_device.bind_render_target(sprite_rt);
        window.render_device.clear_color(Vector4::new(0.0, 0.0, 0.0, 0.0));
        window.render_device.clear();
        window.render_device.bind_shader_program(&mut texture2d_shader);
        static_transform.set_position_xy(-150.0, 0.0);
        window.render_device.draw_texture2d(static_transform.clone(), &mut smiley, Vector4::new(1.0, 1.0, 1.0, 1.0));
        static_transform.set_position_xy(150.0, 0.0);
        window.render_device.draw_texture2d(static_transform.clone(), &mut smiley, Vector4::new(1.0, 1.0, 1.0, 1.0));
        window.render_device.unbind_render_target();

        // Render text to the text render target.
        window.render_device.resize_render_target(&mut text_rt, viewport.size.x, viewport.size.y);
        window.render_device.bind_render_target(text_rt);
        window.render_device.clear_color(Vector4::new(0.0, 0.0, 0.0, 0.0));
        window.render_device.clear();
        window.render_device.bind_shader_program(&mut font_shader);
        window.render_device.draw_text2d(Vector2::new(0.0, 0.0), "Hello gfx!", 1.0, Vector4::new(1.0, 1.0, 1.0, 1.0), &mut font, TextAlignment::MiddleCenter);
        window.render_device.unbind_shader_program();
        window.render_device.unbind_render_target();

        // Render rectangles to the rectangle render target.
        window.render_device.resize_render_target(&mut rect_rt, viewport.size.x, viewport.size.y);
        window.render_device.bind_render_target(rect_rt);
        window.render_device.clear_color(Vector4::new(0.0, 0.0, 0.0, 0.0));
        window.render_device.clear();
        window.render_device.bind_shader_program(&mut solid_rect_shader);
        let rect_transform: Transform2D = Transform2D::new(Vector2::new(0.0, 0.0), 0.0, Vector2::new(128.0, 128.0));
        window.render_device.fill_rect(rect_transform, Vector4::new(0.14, 0.14, 0.14, 1.0));
        window.render_device.bind_shader_program(&mut rect_shader);
        window.render_device.draw_rect(rect_transform, 2.5, Vector4::new(1.00, 0.80, 0.61, 1.0));
        window.render_device.unbind_shader_program();
        window.render_device.unbind_render_target();
        ...
        window.render_device.draw_render_target(rect_rt);
        window.render_device.draw_render_target(text_rt);
        window.render_device.draw_render_target(animated_sprite_rt);
        window.render_device.unbind_shader_program();

        //Swap the buffers and display the render result
        window.swap_buffers();

        // Calculate and display frame time.
        let ellapsed = ellapsed_time.elapsed().as_millis();
        ellapsed_time = Instant::now();
        println!("Rendered frame within {} ms. With error {}", ellapsed, window.render_device.get_error());
    }

    //Cleanup the shaders, images, render_targets and the renderer itself
    ...
    window.render_device.dispose_shader_program(&mut texture_batch_shader);
    window.render_device.dispose_shader_program(&mut font_shader);
    window.render_device.dispose_shader_program(&mut solid_rect_shader);
    window.render_device.dispose_shader_program(&mut rect_shader);
    ...
    window.render_device.dispose_render_target(&mut rect_rt);
    window.render_device.dispose_render_target(&mut text_rt);
    window.render_device.dispose_image_texture(&mut image);
    window.render_device.dispose_image_texture(&mut smiley);
